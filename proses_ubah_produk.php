...
if($_POST){
    ...
    if (empty($nama_produk)){
        ...
        echo "<script>alert('harga produk tidak boleh kosong');location.href='tampil_produk.php';</script>";
    } elseif (empty($foto_produk)){
        echo "<script>alert('foto produk tidak boleh kosong');location.href='tampil_produk.php';</script>";
    } else {
        include "koneksi.php";
        $update = mysqli_query($conn, "update produk set nama_produk='".$nama_produk."', deskripsi='".$deskripsi."', harga='".$harga."', foto_produk='".$foto_produk."' where id_produk = '".$id_produk."'") or
